...
* 方法二
  * 使用查找表来解决该问题。（哈希表）
  * 创建字典record, 字典的key是数组值,value是位置.（逆序存放是为了更好地索引到位置）
  * 遍历数组nums
  
    * 每次遍历时使用临时变量 temp用来保存目标值与当前值的差值
    * 在此次遍历中查找 record ，查看是否有与 temp一致的值，如果查找成功则返回查找值的索引值与当前变量的值 i
    * 如果未找到，则在 record 保存该元素作为key与索引值index作为value
...
* **双指针法**（双指针代表的是 可以作为容器边界的所有位置的范围）
* 定义左右指针初始在边界，计算容量。
* **移动对应数字较小的指针**(left++/right--)，更新容量

### 12、整数转罗马数字[Medium]

==贪心算法==

* 原因：对应数字较小的指针已不能再作为容器的边界（因为移动另外一个的时候，该边界的长度已经固定，短板效应）
...
* **动态规划**——状态、转移方程、初始条件、边界处理
* **转移方程**：当前位置的路径和为左边点和上边点的路径可能的和
...
* **动态规划：** dp[i] = max(dp[i-1], price[i]-minprice)  其中，dp[i]代表当时时刻卖出得到的最大利润。
* **一次遍历法：**
  * 扫描到第i个元素途中，进行更新得到最小价格minprice
  * 描到第i个元素时,如果在这一天卖股票,盈利就是profit = prices[i] - minprice
  * 记录最大值max(profit)

### 122、买卖股票的最佳时机II[Easy]

* **贪心算法——当前价格比昨天高就加入收益**，做出最优的局部选择。即找到所有差分为正数的值，进行累加。
* **动态规划**——定义状态数组cash, stock分别表示持有现金、股票时的收益。之后根据状态转移方程更新状态数组。
